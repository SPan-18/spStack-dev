...
indexed by spatial coordinates sampled uniformly from the unit square.
The model includes one covariate and spatial random effects induced by a
Matérn covariogram.
...
\lambda(s_i) &= x(s_i)^\top \beta + z(s_i) \end{aligned} \$\$ where the
spatial effects \\z \sim N(0, \sigma^2 R)\\ with \\R\\ being a \\n
\times n\\ correlation matrix given by the Matérn covariogram \$\$ R(s,
s') = \frac{(\phi \|s-s'\|)^\nu}{\Gamma(\nu) 2^{\nu - 1}} K\_\nu(\phi
\|s-s'\|), \$\$ where \\\phi\\ is the spatial decay parameter and
\\\nu\\ the spatial smoothness parameter. We have sampled the data with
\\\beta = (2, -0.5)\\, \\\phi = 5\\, \\\nu = 0.5\\, and \\\sigma^2 =
0.4\\. This data can be generated with the code as given in the example
below.
