...
#'  the variables are taken from \code{environment(formula)}, typically the environment from which
#'  \code{spLM_stack} is called.
#' @param coords an \eqn{n \times 2}{n x 2} matrix of the observation coordinates in \eqn{R^2}{R^2}
#'  (e.g., easting and northing).
#' @param cor.fn a quoted keyword that specifies the correlation function used to model the spatial
#'  dependence structure among the observations. Supported covariance model key words are: \code{"exponential"}
#'  and \code{"matern"}. See below for details.
...
#' @param loopd logical. If `loopd=TRUE`, returns leave-one-out predictive densities, using method as
#'  given by \code{loopd_method}.
#' @param loopd_method character. Ignored if `loopd=FALSE`. If `loopd=TRUE`, valid inputs are
#'  `"exact"`, `"PSIS"`. The option `"exact"` corresponds to exact leave-one-out predictive
#'  densities which fit the model \eqn{n} times. The options `"CV"` and `"PSIS"` are fast, and finds
#'  approximate leave-one-out predictive densities using \eqn{k}-fold cross-validation and Pareto-smoothed
#'  importance sampling (Vehtari et. al, 2024), respectively.
#' @param CV_k Only considered if \code{loopd_method = "CV"}. It specifies the value of \eqn{k} for
#'  \eqn{k}-fold cross-validation. \eqn{k} must be an integer between \eqn{10} and \eqn{20} and is
#'  recommended only when \eqn{n > 40}. Otherwise, use \code{loopd_method = "exact"}.
#' @param verbose logical. If \code{verbose = TRUE}, prints model description.
#' @param ... currently no additional argument
#' @references Vehtari A, Simpson D, Gelman A, Yao Y, Gabry J (2024). “Pareto Smoothed Importance Sampling.”
#'  *Journal of Machine Learning Research*, **25**(72), 1–58. URL \url{https://jmlr.org/papers/v25/19-556.html}.
#' @export
spLMexactLOO <- function(formula, data = parent.frame(), coords,
                         cor.fn, priors, spParams, noise_sp_ratio,
                         n.samples, loopd, loopd_method, CV_k, verbose = TRUE, ...){
